...
<#
.SYNOPSIS
    Windows installer for BenchDash via Docker Desktop.
#>
param(
    [switch]$SkipStart
)
...
Write-Host "=== Installing BenchDash ===" -ForegroundColor Cyan
...
if (-not $SkipStart) {
    Write-Host "Building and starting BenchDash on port 8080..." -ForegroundColor Green
    Invoke-Expression "$composeCmd up --build -d"
    Write-Host "BenchDash installed. Access: http://localhost:8080" -ForegroundColor Green
} else {
